...
`PourGlyph` 가 붓는 방법을 짧은 반복 애니메이션으로 그립니다(인라인 SVG + SMIL). 이 애니메이션이
이 기능의 핵심입니다 — 단계 표에서도, 타이머 카드에서도 **열자마자 움직여야** 합니다. 붓는 정보가 없는
단계(뜸 등)에서 타이머 카드는 다음 푸어를 미리 재생합니다. 속도(`pourCycleSec`)는 천천히/빠르게가
눈으로 구분되게 3배 이상 벌려 둡니다(테스트가 지킵니다).

SMIL 의 `dur` 는 `calc()` 를 받지 않으니 숫자로 넘기세요. 기기의 '동작 줄이기'면 멈춘 그림이지만,
설정의 "항상 움직이기"(`usePourMotion`)로 재생할 수 있습니다 — 장식이 아니라 정보라서.
