...
const toneClass = {
  early: 'border-ice/25 bg-ice-soft text-ice',
  good: 'border-sage/25 bg-sage-soft text-sage',
  late: 'border-line-strong bg-card text-ink-soft',
} as const;
    ...
    return (
      <article
        ...
        className={`rounded-2xl border p-4 ${
          bean.finished
            ? 'border-line bg-well opacity-60'
            : active
              ? 'border-crema bg-card'
              : 'border-line bg-card shadow-card'
        }`}
      >
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            <h3 className="truncate font-bold text-ink">{bean.name}</h3>
        ...
        {myGrinder && (
          <p className="mt-3 flex items-baseline justify-between gap-2 rounded-lg bg-well px-3 py-2 text-xs">
            <span className="text-ink-faint">{myGrinder.name} V60 기준</span>
            {anchor !== null ? (
              <span className="num text-sm font-bold text-ink">{anchor}클릭</span>
            ) : (
              <span className="text-ink-faint">미지정 · 기본값 사용</span>
            )}
          </p>
        )}
          ...
          <button
            ...
            className={`mt-3 w-full rounded-lg border py-2 text-xs font-bold transition ${
              active
                ...
                : 'border-line-strong bg-well text-ink-soft hover:bg-line'
            }`}
          >
    ...
    <>
      {beans.length === 0 ? (
        ...
        <div className="space-y-6">
          ...
          {finished.length > 0 && (
            <section>
              <h3 className="mb-2 text-xs font-bold tracking-wider text-ink-faint uppercase">다 마신 원두</h3>
  ...
  return (
    <Modal open onClose={onClose} label={initial ? '원두 수정' : '원두 등록'}>
      ...
      <div className="min-h-0 flex-1 space-y-3 overflow-y-auto p-5">
        ...
        <label className="flex items-center gap-2 pt-1 text-sm font-semibold text-ink-soft">
          <input type="checkbox" checked={finished} onChange={(e) => setFinished(e.target.checked)} className="h-4 w-4 accent-crema" />
          다 마셨음
        </label>

        {error && <p role="alert" className="rounded-xl border border-danger/30 bg-danger-soft px-4 py-3 text-sm font-semibold text-danger">{error}</p>}
      </div>

      <footer className="flex shrink-0 gap-2 border-t border-line bg-card p-4">
        {onDelete && (
          <button type="button" onClick={onDelete} aria-label="원두 삭제" className="rounded-xl border border-danger/30 bg-danger-soft px-4 py-3 font-bold text-danger hover:bg-danger-soft">
