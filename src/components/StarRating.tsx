...
export function StarRating({ value, onChange, size = 20 }: Props) {
  ...
  return (
    <div className={readOnly ? 'flex gap-0.5' : 'flex gap-1'} role={readOnly ? 'img' : 'radiogroup'} aria-label={`별점 ${value ?? 0} / 5`}>
      {[1, 2, 3, 4, 5].map((n) =>
        readOnly ? (
          <Icon key={n} name="star" size={size} filled={(value ?? 0) >= n} className={(value ?? 0) >= n ? 'text-amber-400' : 'text-stone-700'} />
        ) : (
          <button
            key={n}
            type="button"
            ...
            aria-label={`${n}점`}
            onClick={() => onChange(n)}
            className={`rounded p-0.5 transition ${(value ?? 0) >= n ? 'text-amber-400' : 'text-stone-600 hover:text-stone-400'}`}
          >
